def get_reference_type(print_message=True):
    while True:
        reference_type = input("Enter reference type (article, book, inproceedings) or exit: ").lower()
...
def ask_fields(reference_type):
    reference_key = input("Enter key for reference: ")  # lähdeviitteen ensimmäinen kenttä @...
